...
with MergePythonSDK.hris.ApiClient(configuration) as api_client:
    ...
    remote_fields = "location_type" # str | Which fields should be returned in non-normalized form. (optional) if omitted the server will use the default value of "location_type"
...
with MergePythonSDK.hris.ApiClient(configuration) as api_client:
    ...
    # and optional values
    try:
        api_response = api_instance.locations_retrieve(id, include_remote_data=include_remote_data, remote_fields=remote_fields)
        pprint(api_response)
    except MergePythonSDK.hris.ApiException as e:
        print("Exception when calling LocationsApi->locations_retrieve: %s\n" % e)
```
