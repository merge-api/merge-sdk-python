...
with MergePythonSDK.ats.ApiClient(configuration) as api_client:
    ...
    # and optional values
    try:
        api_response = api_instance.offers_retrieve(id, expand=expand, include_remote_data=include_remote_data, remote_fields=remote_fields)
        pprint(api_response)
    except MergePythonSDK.ats.ApiException as e:
        print("Exception when calling OffersApi->offers_retrieve: %s\n" % e)
```
